...
async function ensureInitialized(db) {
  ...
  if (!profile) {
    ...
    await db.collection('mosque_profile').insertOne({
      ...
      description: 'Mushola Al-Iman adalah tempat ibadah yang berdiri sejak tahun 2025.',
...
export async function GET(request, { params }) {
  ...
  try {
    ...
    if (route === '/mosque-profile') {
      ...
      const data = await db.collection('mosque_profile').findOne({}, { projection: { _id: 0 } })
    ...
    if (route === '/prayer-schedule') {
      ...
      const data = await db.collection('prayer_schedule').findOne({}, { projection: { _id: 0 } })
    ...
    if (route === '/social-media') {
      ...
      const data = await db.collection('social_media').findOne({}, { projection: { _id: 0 } })
